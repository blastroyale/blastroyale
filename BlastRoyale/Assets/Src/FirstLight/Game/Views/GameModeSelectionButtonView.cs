...
namespace FirstLight.Game.Views
{
	...
	public class GameModeSelectionButtonView : UIView
	{
		...
		{
			var mutators = GameModeInfo.Entry.MatchConfig.Mutators.GetSetFlags();
		...
		private void SetMutatorLine(VisualElement mutatorLine, Mutator mutator)
		{
			mutatorLine.ClearClassList();
			mutatorLine.AddToClassList(GameModeButtonMutatorLine);
			mutatorLine.AddToClassList(mutator.ToString().ToLowerInvariant() + "-mutator");
			var mutatorTitle = mutatorLine.Q<Label>("Title").Required();
			mutatorTitle.text = mutator.GetLocalizationKey().LocalizeKey();
		}
	}
}