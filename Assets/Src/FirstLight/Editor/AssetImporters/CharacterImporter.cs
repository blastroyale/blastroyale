...
namespace FirstLight.Editor.AssetImporters
{
	public class CharacterImporter: AssetPostprocessor
	{
		...
		private void OnPreprocessModel()
		{
			...
			if (assetPath.Contains("Shared")) return; // Ignore shared folder

			// TODO: Rename
			//var destPath = assetPath.Remove(assetPath.LastIndexOf('/'));
			//importer.ExtractTextures(destPath);
		...
		private void OnPreprocessTexture()
		{
			...
			if (!assetPath.StartsWith(CHAR_PATH)) return;
			if (assetPath.Contains("Shared")) return; // Ignore shared folder
			if (!Path.GetFileName(assetPath).StartsWith("T_Char_")) return; // Only character textures are processed here
