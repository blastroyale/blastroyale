...
namespace FirstLight.UiService
{
	...
	public class UiService : IUiServiceInit
	{
		...
		public UiPresenter OpenUi<TData>(Type type, TData initialData) where TData : struct
		{
			...
			FLog.Verbose("UIService",$"OpeningUI<{type.Name},{typeof(TData).Name}>");
			return OpenUi(type);
		}
		...
		public async Task CloseUi(Type type, bool destroy = false)
		{
			if (!_visibleUiList.Contains(type))
			{
				FLog.Verbose("UIService",$"Is trying to close the {type.Name} ui but is not open");
				return;
			}
			...
			// ReSharper disable once MethodHasAsyncOverload
			var ui = GetUi(type); 
			FLog.Verbose("UIService",$"ClosingUI<{type.Name}>");
		...
		public async Task<UiPresenter> OpenScreenAsync<T>() where T : UiPresenter
		{
			...
			if (_lastScreen != null)
			{
				...
				FLog.Verbose("UIService",$"ClosingPreviousScreen<{_lastScreen.GetType().Name}>");
				await CloseUi(_lastScreen.GetType(), true);
			}
			FLog.Verbose("UIService",$"OpeningScreen<{typeof(T).Name}>");
		...
		public async Task<T> OpenScreenAsync<T, TData>(TData initialData) where T : UiPresenter, IUiPresenterData where TData : struct
		{
			...
			FLog.Verbose("UIService",$"OpeningScreen<{typeof(T).Name},{typeof(TData).Name}>");
		...
		public async void CloseCurrentScreen()
		{
			if (_lastScreen != null)
			{
				FLog.Verbose("UIService",$"ClosingScreen<{_lastScreen.GetType().Name}>");
				await CloseUi(_lastScreen.GetType(), true);
			}
		}
