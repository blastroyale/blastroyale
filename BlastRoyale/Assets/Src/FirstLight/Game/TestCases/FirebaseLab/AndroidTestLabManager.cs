...
namespace FirstLight.Game.TestCases.FirebaseLab
{
	internal class AndroidTestLabManager : TestLabManager
	{
		...
		{
			if (_logFileDescriptor == -1)
			{
				Debug.LogWarning("[FTL] - log file descriptor not initialized, skipping log entry");
				return;
			}

			try
			{
				LibcWrapper.WriteToFileDescriptor(_logFileDescriptor, line + "\n");
			}

			catch (Exception e)
			{
				Debug.LogError("[FTL] - exception writing to log file descriptor: "
					+ e
					+ "\n"
					+ e.StackTrace);
			}
