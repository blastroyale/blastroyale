...
namespace Quantum
{
	public unsafe partial struct Chest
	{
		...
		public void Open(Frame f, EntityRef e, EntityRef playerEntity, PlayerRef player)
		{
			var playerData = f.GetPlayerData(player);
			var chestPosition = f.Get<Transform3D>(e).Position;

			var loadoutWeapon = playerData.Loadout.FirstOrDefault(item => item.IsWeapon());
		...
		private void HandleCollectedStash(Frame f, EntityRef e, int stashValue)
		{
			...
			switch (stashValue)
			{
				// Legendary stash
				case 3:
					Collectable.DropConsumable(f, weaponIDs[f.RNG->Next(0, weaponIDs.Count)], stashPosition, 0, true);
					Collectable.DropConsumable(f, weaponIDs[f.RNG->Next(0, weaponIDs.Count)], stashPosition, 1, true);
				...
				default:
					...
					{
						Collectable.DropConsumable(f, weaponIDs[f.RNG->Next(0, weaponIDs.Count)], stashPosition, 0,
						                            true);
					}
