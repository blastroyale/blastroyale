...
{
	/// <summary>
	/// This editor class helps to find specified or missing components in the project
	/// </summary>
						...
						switch (selectedCheckType)
						{
							...
							case 1:
								...
								if (EditorSceneManager.SaveCurrentModifiedScenesIfUserWantsTo())
								{
										...
										if (indexCS != -1 || indexJS != -1)
										{
											...
											try
											{
												System.IO.FileStream FS =
													new System.IO.FileStream(projectPath + asset,
												...
												while (!SR.EndOfStream)
												{
													...
													if (index != -1)
													{
														...
														else
														{
															newComp =
																new
																	ComponentNames(line.Substring(index + 1, line.Length - (index + 1)),
												    ...
												    dependedAsset.IndexOf(".js") != -1)
												{
													ComponentNames newComponent =
														new ComponentNames(NameFromPath(dependedAsset),
														                   GetNamespaceFromPath(dependedAsset),
														                   dependedAsset);
									...
									{
										addedComponents[i].assetPath =
											GetPathFromNames(addedComponents[i].namespaceName,
										...
										foreach (GameObject g in sceneGOs)
										{
											...
											foreach (Component c in comps)
											{
												    ...
												    c.GetType().BaseType == typeof(MonoBehaviour))
												{
													...
													ComponentNames newComp =
														new ComponentNames(NameFromPath(path),
				...
				if (listResult != null)
				{
					if (listResult.Count == 0)
					{
						GUILayout.Label(editorMode == 0
							                ? (componentName == ""
								                   ? "Choose a component"
								                   : "No prefabs use component " + componentName)
							                : ("No prefabs have missing components!\nClick Search to check again"));
					}
					else
					{
						GUILayout.Label(editorMode == 0
							                ? ("The following " + listResult.Count + " prefabs use component " +
							                   componentName + ":")
