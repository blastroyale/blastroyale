...
namespace FirstLight.Game.UIElements
{
	...
	public class CryptoCurrenciesDisplayElement : VisualElement
	{
		...
		public CryptoCurrenciesDisplayElement()
		{
			...
			_bankContainer = new VisualElement() {name = "PartnerCryptoCurrenciesContainer"};
			_bankContainer.AddToClassList(USS_CRYPTO_CURRENCY_BLOCK);
			_bankContainer.Add(_partnerCryptoCurrenciesArrow);
		
			Add(_bankContainer);

			_bankContainer.SetVisibility(false);
		}
		...
		public void AnimateCurrencyEffect(GameId id, ulong previous, ulong amount, CancellationToken cc, bool text = true)
		{
			...
			_animationHandler.AnimateCurrency(previous, amount, text).Forget();
		}
		...
		{
			if (_cryptoCurrenciesDict.Count == 0)
			{
		...
		{
			_bankContainer.SetVisibility(!_bankContainer.visible);
		}
		...
		public void SetData(int onChainValue, Dictionary<GameId, ulong> playerCryptoCurrenciesDict)
		{
			_cryptoCurrenciesDict = playerCryptoCurrenciesDict;
			var shouldHideElement = onChainValue == 0 && playerCryptoCurrenciesDict.Count == 0;

			this.SetDisplay(!shouldHideElement);

			if (!shouldHideElement)
			{
				SetupCurrenciesDisplay(onChainValue, playerCryptoCurrenciesDict);
			}
		}
		...
		private void SetupCurrenciesDisplay(int onChainValue, Dictionary<GameId, ulong> cryptoCurrenciesDict)
		{
			...
			{
				FLog.Verbose("No currencies in bank to display");
				return;
			}
...
*/
		...
		private void SetupCryptoTokenScrollView(Dictionary<GameId, ulong> cryptoCurrenciesDict)
		{
			...
			_bankContainer.Add(cryptoTokenScrollView);
		}
		...
		private void SetupCryptoTokenColumns(Dictionary<GameId, ulong> cryptoCurrenciesDict)
		{
			...
			foreach (var cryptoCurrency in cryptoCurrenciesDict)
			{
				if (cryptoTokensAdded % 4 == 0)
				{
					...
					{
						_bankContainer.Add(currentCryptoTokenColumn);
					}
